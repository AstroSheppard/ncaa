...
def simulate_games(teams):
    ...
    lose=[]
    # Iterate through every other team and determine probability of first team winning
...
# Function that simulates points for each player given who wins the games. Outputs dataframe
# with player name and total points (in order)
...
def get_hists(df, player, payouts, direct):
    ...
    for i in range(nPaid):
        money=[payouts[i] if rank[j] == i+1 else money[j] for j in range(len(rank))]
    ...
    fig.tight_layout()
    plt.text(0.3, 0.9, "Average won = $" + str(mean_cash)+" +/- "+str(error_cash),
    ...
    plt.clf()
    return money_info
def get_family_prob(results, fam):
    df=pd.DataFrame()
    win=pd.DataFrame()
    for name in fam:

        b=results[results['Player']==name]['Rank']
        val=[1 if item<6 else 0 for item in b]
        valwin=[1 if item==1 else 0 for item in b]
        df[name]=val
        win[name]=valwin
    
    df['cash']=df.sum(axis=1)
    df['cash']=[1 if item>0 else 0 for item in df['cash']]
    total=df['cash'].sum()
    length=len(df['cash'])
    prob=float(total)/length

    win['cash']=win.sum(axis=1)
    win['cash']=[1 if item>0 else 0 for item in win['cash']]
    total=win['cash'].sum()
    length=len(win['cash'])
    prob_win=float(total)/length
...
brackets=pd.read_csv(direct+'/bracket_data.csv') # change
...
if direct == 'yahoo':
    ...
    probs['team_name'].values[l]=probs['team_name'][l].replace('Loyola (IL)', 'Loyola Chicago')
    probs['team_name'].values[k]=probs['team_name'][k].replace('Kansas State', 'Kansas St.')
    probs['team_name'].values[f]=probs['team_name'][f].replace('Florida State', 'Florida St.')
else:
    probs['team_name'].values[l]=probs['team_name'][l].replace('Loyola (IL)', 'Loyola-Chicago')
  
probs['team_name']=probs['team_name'].str.replace('Texas A&M', 'Texas A&M;')   
...
players=brackets['Bracket Name'].drop_duplicates()

family_name=[players.values[5], players.values[25], players.values[12], players.values[6]]
print family_name
fam=get_family_prob(results, family_name)
print fam
sys.exit()
