...
# generate games
# same as simulate_games, but prob is now = to pick percentage
   ...
   #         print i
    ...
    if direct == 'yahoo':
        for i in probs.index:
            probs.loc[i, 'team_name']=probs.loc[i, 'team_name'].replace('State', 'St.')
        f=probs.index[probs['team_name']=='North Carolina'][0]
        probs['team_name'].values[f]=probs[
            'team_name'][f].replace('North Carolina', 'N. Carolina')
        ...
        probs['team_name'].values[f]=probs[
            'team_name'][f].replace('Saint Mary\'s (CA)', 'St. Mary\'s')
        f=probs.index[probs['team_name']=='Virginia Commonwealth'][0]
        probs['team_name'].values[f]=probs[
            'team_name'][f].replace('Virginia Commonwealth', 'VCU')
        f=probs.index[probs['team_name']=='Louisiana St.'][0]
        probs['team_name'].values[f]=probs[
            'team_name'][f].replace('Louisiana St.', 'LSU')
        f=probs.index[probs['team_name']=='New Mexico St.'][0]
        probs['team_name'].values[f]=probs[
            'team_name'][f].replace('New Mexico St.', 'N. Mex. St.')
